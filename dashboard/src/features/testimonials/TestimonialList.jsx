...
export default function TestimonialList() {
  ...
  const query = testimonials.useList(reordering ? REORDER_PARAMS : list.params)
  ...
  const columns = [
    { key: 'order', header: c.order, sortable: true, width: 88, cell: (row, index) => position(index) },
    {
      ...
      cell: (row) => (
        <div className="flex min-w-0 items-center gap-3">
          <Avatar src={row.avatar_url} name={row.name} alt="" />
    ...
    { key: 'company', header: t.company, sortable: true, hideBelow: 'md', cell: (row) => (row.company ? <span dir="auto">{row.company}</span> : null) },
    // the excerpt shows the quote in the UI language (the other one is one click away in the edit form)
    { key: 'quote', header: t.quote, hideBelow: 'xl', cell: (row) => <p className="max-w-xs truncate text-muted">{pickField(row, 'quote')}</p> },
    { key: 'rating', header: t.rating, sortable: true, hideBelow: 'sm', cell: (row) => <Stars value={row.rating} /> },
    {
      key: 'is_active',
      header: c.status,
      sortable: true,
          ...
          <SortableList
            ...
            renderItem={(row, { handle, isDragging, index }) => (
              <div className={`flex items-center gap-3 rounded-xl border bg-white/[.03] px-3 py-2 ${isDragging ? 'border-accent-light' : 'border-white/[.08]'}`}>
                ...
                <Avatar src={row.avatar_url} name={row.name} alt="" size="sm" />
