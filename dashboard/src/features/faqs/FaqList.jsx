...
export default function FaqList() {
  ...
  const query = faqs.useList(reordering ? { per_page: REORDER_LIMIT, sort: 'order', dir: 'asc' } : list.params)
  ...
  const onDelete = async (row) => {
    const ok = await confirm({ title: t.deleteTitle, message: t.deleteMessage(pickField(row, 'question')), confirmLabel: c.delete })
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="min-w-0 max-w-xl">
          <p className="truncate font-semibold">{pickField(row, 'question')}</p>
          <p className="truncate text-xs text-muted">
            <bdi dir={otherLang === 'ar' ? 'rtl' : 'ltr'} lang={otherLang}>
              {row[`question_${otherLang}`]}
            </bdi>
          </p>
        </div>
      ),
    },
    {
      key: 'is_active',
      header: c.status,
      cell: (row) => <Switch checked={Boolean(activeOf(row))} onChange={(is_active) => update.mutate({ id: row.id, data: { is_active } })} aria-label={`${c.active}: ${pickField(row, 'question')}`} />,
    },
    { key: 'created_at', header: c.createdAt, sortable: true, hideBelow: 'md', cell: (row) => f.date(row.created_at) },
  ]

  const rowActions = (row) => [
    { key: 'edit', label: c.edit, icon: Pencil, to: `/faqs/${row.id}` },
    { key: 'delete', label: c.delete, icon: Trash2, tone: 'danger', onClick: () => onDelete(row), disabled: remove.isPending },
  ]

  const newButton = (
    <Button to="/faqs/new" icon={Plus}>
      {t.new}
    </Button>
  )
          ...
          <SortableList
            ...
            renderItem={(row, { handle, isDragging, index }) => (
              <div className={`flex items-center gap-3 rounded-xl border bg-white/[.03] px-3 py-2 ${isDragging ? 'border-accent-light' : 'border-white/[.08]'}`}>
                ...
                <p className="min-w-0 flex-1 truncate text-sm font-semibold">{pickField(row, 'question')}</p>
